...
检测常规位置，找不到就报告错误。入口不会自动结束已打开的桌面程序；需要切换连接时，
由用户正常退出后重新打开。未配置自动接入的安装仍需使用 Pocket 的“连接桌面 App”。
...
前置依赖要求不变。安装期间已运行的 App 需要正常退出并重开一次。
...
不调用模型，不操作已有任务。已生成的本机代理入口也已更新；已运行的代理进程需在
桌面 App 下次通过 Pocket 连接时重新加载。此验证不等同于桌面 GUI 或 Windows 真机验收。

- 当前桌面 App 仍在独立连接中执行本次修改，首次共享模式的桌面窗口验收需正常退出后
  从共享入口重新打开。桌面专属插件、工具及真实模型审批尚未完成共享模式的 GUI 验收。
...
后代进程连接了共享端口后才显示“已共享”。发现桌面直接启动独立 CLI 时显示“桌面未接入”，
