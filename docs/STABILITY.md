...
本记录下方保留前一轮稳定性检查；本次桌面与 Web 同任务切换、共享后端、状态竞态、
附件归属和最新测试结果见 [跨端切换检查](cross-platform-switching.md)。
...
- 独立模式下，Mac Codex App 的桌面接口可能要求签名身份，单独恢复 Pocket 连接不能
  接管桌面持有的任务。共享模式通过桌面和 Web 使用同一后端解决写入冲突，需要第一次
  从共享入口重新打开桌面 App，没有绕过签名校验。
